...
class GenerateFactors:
    ...
    def _init_dir(self):
        ...
        # 创建专门的org数据目录
        self.org_dir = self.factor_dir / self.ind_cate / 'org'
    ...
    def _load_param(self):
        ...
        # 获取是否保存原始(org)数据的配置
        self.save_org = self.params.get('save_org', True)
    ...
    def _get_aggregation_mapping(self):
        ...
        for agg_name, agg_info in aggregation_info.items():
            ...
            aggregation_mapping[agg_name] = partial(agg_func, daily_weights=self.daily_weights, 
                                                    ...
                                                    imb_func=imb_func, ts_func_with_pr=ts_func_with_pr,
                                                    cs_func=cs_func, n_workers=self.n_workers)
...
# 修改generate_one函数以支持第二层时序变换和org因子处理逻辑
def generate_one(ind_name, ind_dir, target_dir, org_dir, aggregation_mapping, ts_mapping, 
                mode='init', ts_second_layer_mapping=None, save_first_layer=True, save_org=True):
    try:
    ...
    for agg_name, agg_func in aggregation_mapping.items():
        ...
        # 构建所有可能的路径组合：target目录和org目录
        target_factor_paths = {output_type: {tag: target_dir / f'{factor_name}-org.parquet'
                                             for tag, factor_name in factor_names.items()}
                               for output_type, factor_names in possible_factor_names.items()}
        
        org_factor_paths = {output_type: {tag: org_dir / f'{factor_name}-org.parquet'
                                          for tag, factor_name in factor_names.items()}
                            for output_type, factor_names in possible_factor_names.items()}
        
        # 先尝试在target目录找，再尝试在org目录找
        res = None
        for paths_dict in [target_factor_paths, org_factor_paths]:
            for output_type, path_dict in paths_dict.items():
                if all([os.path.exists(path) for path in path_dict.values()]):
                    if output_type == 'single':
                        res = pd.read_parquet(path_dict['single'])
                        print(f"Loading factor from {path_dict['single']}")
                        break
                    elif output_type == 'combo':
                        res = {tag: pd.read_parquet(factor_path) for tag, factor_path in path_dict.items()}
                        print(f"Loading factors from {', '.join(str(p) for p in path_dict.values())}")
                        break
            if res is not None:
                break
        ...
        if isinstance(res, pd.DataFrame):
            ...
            if save_org:
                # 保存到指定的org目录
                factor.to_parquet(org_dir / f'{factor_name}-org.parquet')
                
                # 如果target_dir与org_dir不同，也保存到target_dir
                if str(target_dir) != str(org_dir):
